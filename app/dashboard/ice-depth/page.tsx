...
export default function IceDepthPage() {
  ...
  const fetchFormTemplate = async () => {
    try {
      const response = await fetch('/api/form-templates?moduleType=ICE_DEPTH&isActive=true')
      if (!response.ok) {
      ...
      if (result.templates && result.templates.length > 0) {
        ...
        console.error('No active form template found for ICE_DEPTH module')
      }
    } catch (error) {
  ...
  const saveDraft = async (data: FormData, silent = false) => {
    ...
    try {
      ...
      if (!currentDraftId && result.submission?.id) {
        ...
        router.replace(`/dashboard/ice-depth?draft=${result.submission.id}`)
      }
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    try {
      ...
      alert('Ice Depth measurement submitted successfully!')
      router.push('/dashboard/submissions')
    } catch (error) {
    ...
    return (
      <div className="space-y-6">
        <div className="border-b border-wolf-200 pb-4">
          <h1 className="text-3xl font-bold text-navy">Ice Depth</h1>
          <p className="text-wolf-600 mt-2">Measure ice depth across designated measurement points</p>
        </div>
        <Card>
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-wolf-200 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-navy">Ice Depth Measurement</h1>
            <p className="text-wolf-600 mt-2">
              Record ice depth measurements across the rink surface
            </p>
          </div>
          {currentDraftId && (
