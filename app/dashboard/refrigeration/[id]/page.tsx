...
const OIL_LEVEL_OPTIONS = ['normal', 'low', 'high']
const REFRIGERANT_LEVEL_OPTIONS = ['normal', 'low', 'high']
const SIGHT_GLASS_OPTIONS = ['clear', 'bubbles', 'cloudy']

export default function RefrigerationDetailPage() {
  ...
  const fetchSubmission = async () => {
    try {
      ...
      if (meResponse.ok) {
        ...
        const hasEditPerm = meData.user?.role?.permissions?.refrigeration?.edit
  ...
  if (error && !submission) {
    return (
      <div className="card text-center py-12">
        ...
        <Link href="/dashboard/refrigeration" className="btn btn-secondary">Back</Link>
      </div>
    )
  }
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            ...
            <div className="flex gap-4 mb-4">
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={editData.compressor1Running || false}
                  onChange={(e) => setEditData({ ...editData, compressor1Running: e.target.checked })}
                  className="h-4 w-4 rounded border-gray-300"
                />
                <span className="text-sm">Compressor #1 Running</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={editData.compressor2Running || false}
                  onChange={(e) => setEditData({ ...editData, compressor2Running: e.target.checked })}
                  className="h-4 w-4 rounded border-gray-300"
                />
                <span className="text-sm">Compressor #2 Running</span>
              </label>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Suction PSI</label>
                <input
                  type="number"
                  value={editData.suctionPressure || ''}
                  onChange={(e) => setEditData({ ...editData, suctionPressure: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Discharge PSI</label>
                <input
                  type="number"
                  value={editData.dischargePressure || ''}
                  onChange={(e) => setEditData({ ...editData, dischargePressure: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Oil PSI</label>
                <input
                  type="number"
                  value={editData.oilPressure || ''}
                  onChange={(e) => setEditData({ ...editData, oilPressure: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  value={editData.compressorAmps || ''}
                  onChange={(e) => setEditData({ ...editData, compressorAmps: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
            </div>
            <div className="mt-4">
              ...
              <select
                value={editData.oilLevel || ''}
                onChange={(e) => setEditData({ ...editData, oilLevel: e.target.value })}
                className="input"
              >
                <option value="">Select...</option>
                {OIL_LEVEL_OPTIONS.map(opt => (
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Brine Supply °F</label>
                <input
                  type="number"
                  value={editData.brineTemp || ''}
                  onChange={(e) => setEditData({ ...editData, brineTemp: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Brine Return °F</label>
                <input
                  type="number"
                  value={editData.brineReturnTemp || ''}
                  onChange={(e) => setEditData({ ...editData, brineReturnTemp: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Condenser °F</label>
                <input
                  type="number"
                  value={editData.condenserTemp || ''}
                  onChange={(e) => setEditData({ ...editData, condenserTemp: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Water In °F</label>
                <input
                  type="number"
                  value={editData.condenserWaterIn || ''}
                  onChange={(e) => setEditData({ ...editData, condenserWaterIn: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Water Out °F</label>
                <input
                  type="number"
                  value={editData.condenserWaterOut || ''}
                  onChange={(e) => setEditData({ ...editData, condenserWaterOut: e.target.value ? parseFloat(e.target.value) : null })}
                  className="input"
                />
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div className="grid md:grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <select
                  value={editData.refrigerantLevel || ''}
                  onChange={(e) => setEditData({ ...editData, refrigerantLevel: e.target.value })}
                  className="input"
                >
                  <option value="">Select...</option>
                  {REFRIGERANT_LEVEL_OPTIONS.map(opt => (
                    <option key={opt} value={opt}>{opt.charAt(0).toUpperCase() + opt.slice(1)}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <select
                  value={editData.sightGlass || ''}
                  onChange={(e) => setEditData({ ...editData, sightGlass: e.target.value })}
                  className="input"
                >
                  <option value="">Select...</option>
                  {SIGHT_GLASS_OPTIONS.map(opt => (
                    <option key={opt} value={opt}>{opt.charAt(0).toUpperCase() + opt.slice(1)}</option>
                  ))}
                </select>
              </div>
            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={editData.abnormalNoise || false}
                  onChange={(e) => setEditData({ ...editData, abnormalNoise: e.target.checked })}
                  className="h-4 w-4 rounded border-gray-300"
                />
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={editData.abnormalVibration || false}
                  onChange={(e) => setEditData({ ...editData, abnormalVibration: e.target.checked })}
                  className="h-4 w-4 rounded border-gray-300"
                />
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={editData.leaksDetected || false}
                  onChange={(e) => setEditData({ ...editData, leaksDetected: e.target.checked })}
                  className="h-4 w-4 rounded border-gray-300"
                />
                <span className="text-sm text-gray-700">Leak Detected</span>
              </label>
            </div>
            {editData.leaksDetected && (
              <div className="mt-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">Leak Location</label>
                <input
                  type="text"
                  value={editData.leakLocation || ''}
                  onChange={(e) => setEditData({ ...editData, leakLocation: e.target.value })}
                  className="input"
          ...
          <div className="card">
            ...
            <label className="flex items-center gap-2 mb-4">
              <input
                type="checkbox"
                checked={editData.maintenanceRequired || false}
                onChange={(e) => setEditData({ ...editData, maintenanceRequired: e.target.checked })}
                className="h-4 w-4 rounded border-gray-300"
              />
              <span className="text-sm text-gray-700">Maintenance Required</span>
            </label>
            {editData.maintenanceRequired && (
              <div>
