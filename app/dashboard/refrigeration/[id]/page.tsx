...
export default async function ViewRefrigerationPage({ params }: PageProps) {
  ...
  const canApprove = canUserAccess(user, 'refrigeration', 'approve')
  ...
  const canDelete = submission.userId === user.id && submission.status === 'DRAFT' ||
    canUserAccess(user, 'refrigeration', 'delete')
