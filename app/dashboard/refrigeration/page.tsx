...
export default function RefrigerationPage() {
  ...
  const fetchFormTemplate = async () => {
    try {
      const response = await fetch('/api/form-templates?moduleType=REFRIGERATION&isActive=true')
      if (!response.ok) {
      ...
      if (result.templates && result.templates.length > 0) {
        ...
        console.error('No active form template found for REFRIGERATION module')
      }
    } catch (error) {
  ...
  const saveDraft = async (data: FormData, silent = false) => {
    ...
    try {
      ...
      if (!currentDraftId && result.submission?.id) {
        ...
        router.replace(`/dashboard/refrigeration?draft=${result.submission.id}`)
      }
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    try {
      ...
      alert('Refrigeration report submitted successfully!')
      router.push('/dashboard/submissions')
    } catch (error) {
    ...
    return (
      <div className="space-y-6">
        <div className="border-b border-wolf-200 pb-4">
          ...
          <p className="text-wolf-600 mt-2">Monitor refrigeration system readings and performance</p>
        </div>
        <Card>
    ...
    <div className="space-y-6">
      ...
      <div className="border-b border-wolf-200 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-navy">Refrigeration System Report</h1>
            <p className="text-wolf-600 mt-2">
              Record refrigeration equipment performance and maintenance
            </p>
          </div>
          {currentDraftId && (
