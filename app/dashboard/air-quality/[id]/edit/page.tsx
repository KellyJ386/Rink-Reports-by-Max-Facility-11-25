...
export default async function EditAirQualityPage({ params }: PageProps) {
  ...
  if (submission.status !== 'DRAFT') {
    redirect(`/dashboard/air-quality/${id}`)
  }

  const canEdit = submission.userId === user.id || canUserAccess(user, 'airQuality', 'edit')
  if (!canEdit) {
    redirect(`/dashboard/air-quality/${id}`)
  }
