...
export default function ChecklistsPage() {
  ...
  const fetchFormTemplate = async () => {
    try {
      const response = await fetch('/api/form-templates?moduleType=DAILY_CHECKLIST&isActive=true')
      if (!response.ok) {
      ...
      if (result.templates && result.templates.length > 0) {
        ...
        console.error('No active form template found for DAILY_CHECKLIST module')
      }
    } catch (error) {
  ...
  const saveDraft = async (data: FormData, silent = false) => {
    ...
    try {
      ...
      if (!currentDraftId && result.submission?.id) {
        ...
        router.replace(`/dashboard/checklists?draft=${result.submission.id}`)
      }
    ...
    return (
      <div className="space-y-6">
        <div className="border-b border-wolf-200 pb-4">
          <h1 className="text-3xl font-bold text-navy">Daily Operations Checklist</h1>
          <p className="text-wolf-600 mt-2">Complete daily facility inspection and operational tasks</p>
        </div>
        <Card>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-wolf-600 mt-2">
              Complete daily facility inspection and operational tasks
            </p>
          </div>
          {currentDraftId && (
