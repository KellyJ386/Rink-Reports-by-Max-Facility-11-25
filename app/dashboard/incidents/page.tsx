...
export default function IncidentsPage() {
  ...
  const fetchFormTemplate = async () => {
    try {
      const response = await fetch('/api/form-templates?moduleType=INCIDENTS&isActive=true')
      if (!response.ok) {
      ...
      if (result.templates && result.templates.length > 0) {
        ...
        console.error('No active form template found for INCIDENTS module')
      }
    } catch (error) {
  ...
  const saveDraft = async (data: FormData, silent = false) => {
    ...
    try {
      ...
      if (!currentDraftId && result.submission?.id) {
        ...
        router.replace(`/dashboard/incidents?draft=${result.submission.id}`)
      }
    ...
    return (
      <div className="space-y-6">
        <div className="border-b border-wolf-200 pb-4">
          <h1 className="text-3xl font-bold text-navy">Incident Reports</h1>
          <p className="text-wolf-600 mt-2">Document incidents, injuries, and emergency situations</p>
        </div>
        <Card>
    ...
    <div className="space-y-6">
      <div className="border-b border-wolf-200 pb-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-navy">Incident Report</h1>
            <p className="text-wolf-600 mt-2">
              Document incidents, injuries, near-misses, and emergency situations
            </p>
          </div>
          {currentDraftId && (
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-orange-700">
              <p>Complete incident reports must be filed for:</p>
              <ul className="list-disc list-inside mt-1">
                <li>All injuries requiring first aid or medical attention</li>
                <li>Near-miss events that could have resulted in injury</li>
                <li>Equipment failures affecting safety</li>
                <li>Safety violations or hazardous conditions</li>
              </ul>
              <p className="mt-2 font-semibold">
                For critical incidents, contact facility management immediately.
              </p>
            </div>
          </div>
        </div>
